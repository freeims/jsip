...
import com.github.freeims.freeims.javax.sip.header.*;
 ...
 *
 * @see com.github.freeims.freeims.javax.sip.parser.PipelinedMsgParser
 *
 *
...
public class TLSMessageChannel extends ConnectionOrientedMessageChannel {
     ...
     *
     * @param sock
     *                     Socket from which to read and write messages. The socket
     *                     is
    ...
    protected synchronized void sendMessage(byte[] msg, boolean retry) throws IOException {
        ...
        try {
            ...
            problem = any;
            logger.logWarning(
                    "Failed to connect " + this.peerAddress + ":" + this.peerPort + " but trying the advertised port="
                            + this.peerPortAdvertisedInHeaders + " if it's different than the port we just failed on");
            logger.logError("Error is ", any);
        }
        if (sock == null) { // http://java.net/jira/browse/JSIP-362 If we couldn't connect to the host, try
            ...
            if (peerAddressAdvertisedInHeaders != null && peerPortAdvertisedInHeaders > 0) {
                logger.logWarning("Couldn't connect to peerAddress = " + peerAddress + " peerPort = " + peerPort
                        + " key = " + key + " retrying on peerPortAdvertisedInHeaders " + peerPortAdvertisedInHeaders);
                ...
                this.key = MessageChannel.getKey(peerAddress, peerPort, "TLS");
                logger.logWarning("retry suceeded to peerAddress = " + peerAddress + " peerPortAdvertisedInHeaders = "
    ...
    public synchronized void sendMessage(byte message[], InetAddress receiverAddress,
        ...
        if (peerPortAdvertisedInHeaders <= 0) {
            ...
            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                logger.logDebug("2.Storing peerPortAdvertisedInHeaders = " + peerPortAdvertisedInHeaders
        ...
        if (sock == null) { // http://java.net/jira/browse/JSIP-362 If we couldn't connect to the host, try
                            // the advertised host:port as failsafe
            if (peerAddressAdvertisedInHeaders != null && peerPortAdvertisedInHeaders > 0) {
                logger.logWarning("Couldn't connect to receiverAddress = " + receiverAddress + " receiverPort = "
                        + receiverPort + " key = " + key + " retrying on peerPortAdvertisedInHeaders "
        ...
        if (sock != mySock && sock != null) {
            if (mySock != null) {
                ...
                // we can't delay the close otherwise it will close the previous socket we just
                // set
                close(false, false);
            }
            if (problem == null) {
    ...
    public void run() {
        ...
        // moved from constructor to the TCPMessageChannel thread to avoid blocking the
        // Processor if the handshake is stuck because of a bad client
