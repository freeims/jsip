 ...
 *
 *         <a href=
 *         "{@docRoot}/../uml/udp-request-processing-sequence-diagram.jpg">
 *         See the implementation sequence diagram for processing incoming
 *         requests.
 *         </a>
 *
 *
 *         Acknowledgement: Jeff Keyser contributed ideas on starting and
 *         stoppping the
 *         stack that were incorporated into this code. Niklas Uhrberg suggested
 *         that
    ...
    private void reportSockeException(Exception e) {
        if (exceptionsReportedCounter < MAX_EXCEPTIONS_TO_REPORT) {
            ...
            String msg = String.format(
                    "Caught '%s' on UDP receive socket on %s:%s, message '%s'. Trying to ignore it ... %s",
                    e.getClass().getSimpleName(), sock.getLocalAddress().getHostAddress(), getPort(), e.getMessage(),
                    lastBeforeFloodingChecker
                            ? "(Flooding checker active, no more socket IO-exceptions will be reported)"
                            : "");
