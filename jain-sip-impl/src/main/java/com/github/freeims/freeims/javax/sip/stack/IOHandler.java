...
public class IOHandler {
     ...
     *
     * @param dst
     *                     the destination address that the socket would need to
     *                     connect
     *                     to.
     * @param dstPort
     ...
     *                     with.
     * @param localAddress
     *                     the address that we would like to bind on (null for the
     *                     "any"
     *                     address).
     * @param localPort
            ...
            try {
                ...
                while (retry_count < max_retry) {
                    if (clientSock == null) {
                        ...
                        } catch (SocketException e) { // We must catch the socket timeout exceptions here, any
                                                      // SocketException not just ConnectException
                            logger.logError("Problem connecting " +
                                    receiverAddress + " " + contactPort + " " + senderAddress + " for message "
                                    + new String(bytes, "UTF-8"));
                            // new connection is bad.
                            // remove from our table the socket and its semaphore
                            removeSocket(key);
                            throw new SocketException(
                                    e.getClass() + " " + e.getMessage() + " " + e.getCause() + " Problem connecting " +
                                            receiverAddress + " " + contactPort + " " + senderAddress + " for message "
                                            + new String(bytes, "UTF-8"));
                        }
                        if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
