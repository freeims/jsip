...
public class WebSocketCodec {
	...
	public byte[] decode(InputStream is)
		...
		// All TCP slow-start algorithms will be cut off right here without further
		// analysis
		if (writeIndex < 4) {
		...
		try {
			...
			if (framePayloadLen1 == 126) {
				...
				if (framePayloadLength < 65536) {
					protocolViolation(
							"invalid data frame length (not using minimal length encoding): " + framePayloadLength);
					return null;
				}
			} else {
		...
		if (writeIndex < totalPacketLength) {
			if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
				logger.logDebug("Abort decode. Write index is at " + writeIndex + " and totalPacketLength is "
						+ totalPacketLength);
			}
		...
		// Now move the pending data to the begining of the buffer so we can continue
		// having good stream
