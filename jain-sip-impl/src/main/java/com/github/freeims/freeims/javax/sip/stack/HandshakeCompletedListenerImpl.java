...
public class HandshakeCompletedListenerImpl implements HandshakeCompletedListener {
    ...
    // Added for https://java.net/jira/browse/JSIP-483, NIO doesn't provide a
    ...
    public void startHandshakeWatchdog() {
        if (this.watchdog != null) {
            logger.logInfo("starting watchdog for socket " + watchdog.socket + " on sslhandshake "
                    + sipStack.getSslHandshakeTimeout());
