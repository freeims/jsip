...
import com.github.freeims.freeims.javax.sip.address.GenericURI;
import com.github.freeims.freeims.javax.sip.address.SipUri;
...
import com.github.freeims.freeims.javax.sip.header.RequestLine;
...
import com.github.freeims.freeims.javax.sip.stack.SIPTransactionStack;
...
public class SIPRequest extends SIPMessage implements javax.sip.message.Request, RequestExt {
    ...
    /**
     * Set of target refresh methods, currently: INVITE, UPDATE, SUBSCRIBE, NOTIFY,
     * REFER
     * 
    ...
    /*
     * A table that maps a name string to its cannonical constant. This is used to
     * speed up
     * parsing of messages .equals reduces to == if we use the constant value.
     * 
     * jeand : Setting the capacity to save on memory since the capacity here will
     * never change
     */
     ...
     * comparisons run much
     * faster in the stack because then it is just identity comparision. Character
     * by char
     * comparison is not required. The method returns the String CONSTANT
     ...
     * method converts
     * this into a sip message that is suitable for transmission. Note hack here if
     * you want to
    ...
    /**
     * Check header for constraints. (1) Invite options and bye requests can only
     * have SIP URIs in
     * the contact headers. (2) Request must have cseq, to and from and via headers.
     * (3) Method in
    ...
    public void checkHeaders() throws ParseException {
        ...
        /*
         * RFC 3261 8.1.1.8 The Contact header field MUST be present and contain exactly
         * one SIP
         * or SIPS URI in any request that can result in the establishment of a dialog.
         * For the
         * methods defined in this specification, that includes only the INVITE request.
         * For these
         * requests, the scope of the Contact is global. That is, the Contact header
         * field value
         * contains the URI at which the UA would like to receive requests, and this URI
         * MUST be
        ...
        if (SIPTransactionStack.isDialogCreated(method)) {
             ...
             * For proxy the contact is not relevent, so the UAC behavior RFC 8.1.1.8
             * doesn't apply, we remove this check so double-record-routing proxies will
             * work
             * ....
             * CSeq: 1 INVITE
             * From: <sip:proxy-tls@sip-servlets.com>;tag=5326009
             * To: <sip:receiver@sip-servlets.com>
             * Via: SIP/2.0/TLS 127.0.0.1:5071;branch=z9hG4bKd72cf250-04fc-49a8-bde1-
             * 2b9728ab0dc1_8a474751_1317304087784710000
             * Via: SIP/2.0/TCP
             * 127.0.0.1:5080;branch=z9hG4bK-343139-25f9b919076ced8b4c20506d35036178;rport=
             * 60554
             * Max-Forwards: 69
             * REM: RRRREM
             * Contact: <sip:proxy-tls@127.0.0.1:5080>
             * Record-Route:
             * <sip:127.0.0.1:5071;transport=tls;appname=8a474751;proxy=true;app_id=d72cf250
             * -04fc-49a8-bde1-2b9728ab0dc1;lr>
             * Record-Route:
             * <sip:127.0.0.1:5070;transport=tcp;appname=8a474751;proxy=true;app_id=d72cf250
             * -04fc-49a8-bde1-2b9728ab0dc1;lr>
             * Content-Length: 0
             * 
             * Caused by: java.text.ParseException: Scheme for contact should be
             * sips:sip:proxy-tls@127.0.0.1:5080
             * at gov.nist.javax.sip.message.SIPRequest.checkHeaders(SIPRequest.java:321)
             * at
             * gov.nist.javax.sip.SipProviderImpl.getNewClientTransaction(SipProviderImpl.
             * java:302)
             * ... 23 more
             * 
             ...
             * 
             * if(rrh == null) { // If RRH is not null, then this is proxy so RFC 8.1.1.8
             * doesn't apply, contact doesnt matter when there is RRH
     ...
     * general URI. It
     * indicates the user or service to which this request is being addressed. SIP
     * elements MAY
     * support Request-URIs with schemes other than "sip" and "sips", for example
     * the "tel" URI
     * scheme. SIP elements MAY translate non-SIP URIs using any mechanism at their
     * disposal,
     ...
     * 
     * @return the method from the request line if the method exits and null if the
     *         request line
    ...
    /**
     * Get the message as a linked list of strings. Use this if you want to iterate
     * through the
     * message.
     * 
     ...
     * messages with a
     * pattern and do something when you find a match. This is useful for building
     * filters/pattern
    ...
    /**
     * Encode this into a byte array. This is used when the body has been set as a
     * binary array
    ...
    /**
     * Creates a default SIPResponse message for this request. Note You must add the
     * necessary
     * tags to outgoing responses if need be. For efficiency, this method does not
     * clone the
     * incoming request. If you want to modify the outgoing response, be sure to
     * clone the
     * incoming request as the headers are shared and any modification to the
     * headers of the
     * outgoing response will result in a modification of the incoming request. Tag
     * fields are
     * just copied from the incoming request. Contact headers are removed from the
     * incoming
     * request. Added by Jeff Keyser.
     * 
     * @param statusCode Status code for the response. Reason phrase is generated.
     * 
     * @return A SIPResponse with the status and reason supplied, and a copy of all
     *         the original
     ...
     * @return A SIPResponse with the status and reason supplied, and a copy of all
     *         the original
     *         headers from this request except the ones that are not supposed to be
     *         part of the
     *         response .
     */
    ...
    public SIPResponse createResponse(int statusCode, String reasonPhrase) {
        ...
        // no need to iterate through all headers to create the response since we know
        // which headers
        // we only want to keep and helps the lazy parsing to avoid going through all
        // headers
        for (String headerName : headersToIncludeInResponse) {
            SIPHeader nextHeader = headerTable.get(headerName);
            if (nextHeader != null) {
                if (!(nextHeader instanceof RecordRouteList)
                        || (nextHeader instanceof RecordRouteList && mustCopyRR(statusCode))) {
                    try {
                        newResponse.attachHeader((SIPHeader) nextHeader.clone(), false);
                    } catch (SIPDuplicateHeaderException e) {
    ...
    /**
     * Creates a default SIPResquest message that would cancel this request. Note
     * that tag
     * assignment and removal of is left to the caller (we use whatever tags are
     * present in the
     * original request).
     * 
    ...
    public SIPRequest createCancelRequest() throws SipException {
         ...
         * Request-URI,
         * Call-ID, To, the numeric part of CSeq, and From header fields in the CANCEL
         * request
         * MUST be identical to those in the request being cancelled, including tags. A
         * CANCEL
         * constructed by a client MUST have only a single Via header field value
         * matching the top
         * Via value in the request being cancelled. Using the same values for these
         * header fields
         * allows the CANCEL to be matched with the request it cancels (Section 9.2
         * indicates how
         * such matching occurs). However, the method part of the CSeq header field MUST
         * have a
         * value of CANCEL. This allows it to be identified and processed as a
         * transaction in its
    ...
    /**
     * Creates a default ACK SIPRequest message for this original request. Note that
     * the
     * defaultACK SIPRequest does not include the content of the original
     * SIPRequest. If
     * responseToHeader is null then the toHeader of this request is used to
     * construct the ACK.
     * Note that tag fields are just copied from the original SIP Request. Added by
     * Jeff Keyser.
     * 
    ...
    public SIPRequest createAckRequest(To responseToHeader) {
        ...
        // cloning instead of iterating through headers so that lazy parsers don't have
        // to parse the messages fully
    ...
    public final SIPRequest createErrorAck(To responseToHeader) throws SipException,
        ...
        /*
         * The ACK request constructed by the client transaction MUST contain values for
         * the
         * Call-ID, From, and Request-URI that are equal to the values of those header
         * fields in
         * the request passed to the transport by the client transaction (call this the
         * "original
         * request"). The To header field in the ACK MUST equal the To header field in
         * the
         ...
         * contain a
         * single Via header field, and this MUST be equal to the top Via header field
         * of the
         * original request. The CSeq header field in the ACK MUST contain the same
         * value for the
         * sequence number as was present in the original request, but the method
         * parameter MUST
         * be equal to "ACK".
         */
        ...
        /*
         * If the INVITE request whose response is being acknowledged had Route header
         * fields,
         * those header fields MUST appear in the ACK. This is to ensure that the ACK
         * can be
    ...
    public String getMergeId() {
        ...
        if (fromTag != null) {
            return new StringBuilder().append(requestUri).append(":").append(fromTag).append(":").append(cseq)
                    .append(":")
