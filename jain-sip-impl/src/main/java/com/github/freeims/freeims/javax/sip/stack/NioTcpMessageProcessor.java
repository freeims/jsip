...
public class NioTcpMessageProcessor extends ConnectionOrientedMessageProcessor {
	...
	// public void assignChannelToDestination(HostPort targetHostPort,
	...
	class ProcessorTask implements Runnable {
		...
		public void read(SelectionKey selectionKey) {
			...
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
				logger.logDebug(
						"Got something on nioTcpMessageChannel " + nioTcpMessageChannel + " socket " + socketChannel);
			if (nioTcpMessageChannel == null) {
				if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
					logger.logDebug("Dead socketChannel" + socketChannel + " socket "
		...
		public void write(SelectionKey selectionKey) {
			...
			final NioTcpMessageChannel nioTcpMessageChannel = NioTcpMessageChannel.getMessageChannel(socketChannel);
			if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
				logger.logDebug("Need to write something on nioTcpMessageChannel " + nioTcpMessageChannel + " socket "
						+ socketChannel);
			if (nioTcpMessageChannel == null) {
				if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
					logger.logDebug("Dead socketChannel" + socketChannel + " socket "
			...
			synchronized (pendingData) {
				...
				while (!queue.isEmpty()) {
					...
					try {
						...
						// Shall we perform a retry mechanism in case the remote host connection was
						// closed due to a TCP RST ?
						// https://java.net/jira/browse/JSIP-475 in the meanwhile remove the data from
						// the hashmap
						queue.remove(0);
						pendingData.remove(socketChannel);
						return;
					...
					if (remain > 0) {
						...
						if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
							logger.logDebug("Socket buffer filled and more is remaining" + queue.size() + " remain = "
									+ remain);
						break;
					}
					queue.remove(0);
				...
				if (queue.isEmpty()) {
					if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
						logger.logDebug("We wrote away all data. Setting READ interest. Queue is emtpy now size ="
								+ queue.size());
		...
		public void connect(SelectionKey selectionKey) throws IOException {
			// Ignoring the advice from http://rox-xmlrpc.sourceforge.net/niotut/ because it
			// leads to spinning on my machine
		...
		public void accept(SelectionKey selectionKey) throws IOException {
			...
			// No need for MAX SOCKET CHANNELS check here because this can be configured at
			// OS level
						...
						try {
							switch (change.type) {
								case ChangeRequest.CHANGEOPS:
									...
									if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
										logger.logDebug("Change opts " + change + " selector = " + selector + " key = "
												+ key + " blocking=" + change.socket.isBlocking());
									}
									break;
								case ChangeRequest.REGISTER:
									try {
										...
										if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
											logger.logDebug("NIO register " + change + " selector = " + selector
													+ " blocking=" + change.socket.isBlocking());
										}
	...
	protected synchronized void remove(
		...
		if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
			logger.logDebug(
					Thread.currentThread() + " removing " + ((NioTcpMessageChannel) messageChannel).getSocketChannel()
							+ " from processor " + getIpAddress() + ":" + getPort() + "/" + getTransport());
		}
	...
	public void start() throws IOException {
		...
		selectorThread
				.setName("NioSelector-" + getTransport() + '-' + getIpAddress().getHostAddress() + '/' + getPort());
	}
