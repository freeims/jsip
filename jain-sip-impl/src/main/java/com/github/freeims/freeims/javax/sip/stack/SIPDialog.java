...
import com.github.freeims.freeims.javax.sip.DialogExt;
import com.github.freeims.freeims.javax.sip.ListeningPointImpl;
import com.github.freeims.freeims.javax.sip.SipListenerExt;
...
import com.github.freeims.freeims.javax.sip.header.RAck;
...
import com.github.freeims.freeims.javax.sip.message.MessageFactoryImpl;
...
import com.github.freeims.freeims.javax.sip.parser.AddressParser;
import com.github.freeims.freeims.javax.sip.parser.CallIDParser;
import com.github.freeims.freeims.javax.sip.parser.ContactParser;
import com.github.freeims.freeims.javax.sip.parser.RecordRouteParser;
...
public class SIPDialog implements javax.sip.Dialog, DialogExt {
            ...
            try {
                ...
                if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                    logger.logDebug("SIPDialog::reInviteSender: dialog = " + ctx.getDialog() + " lastTransaction = "
                            + lastTransaction + " lastTransactionState " + lastTransaction.getState());
                }
     ...
     *                                       - throws SipException if IOEx
     *                                       encountered. Otherwise, no
     *                                       exception is propagated.
     * @param releaseAckSem
     *                                       - release ack semaphore.
     * @throws SipException
     *                      if implementation cannot send the ACK Request for any
     *                      other
     *                      reason
     * 
     */
    private void sendAck(Request request, boolean throwIOExceptionAsSipException)
        ...
        // we clone the request that we store to make sure that if getNextHop modifies
        // the request
    ...
    public boolean isAckSeen() {
        ...
        if (lastAckReceivedCSeqNumber == null
            ...
            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                logger.logDebug("SIPDialog::isAckSeen:lastAckReceivedCSeqNumber = " + lastAckReceivedCSeqNumber
                        + " remoteCSeqNumber = " + this.getRemoteSeqNumber());
            }
    ...
    public boolean addTransaction(SIPTransaction transaction) {
        ...
        if (transaction.isInviteTransaction()) {
            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                logger.logDebug("SIPDialog::setLastTransaction:dialog= " + SIPDialog.this + " lastTransaction = "
                        + transaction);
            }
        ...
        try {
            if (transaction.getRequest().getMethod().equals(Request.REFER)
                 ...
                 * dialog then the
                 * NOTIFY MUST include an id parameter in the Event header containing the
                 * sequence number
                 * (the number from the CSeq header field value) of the REFER this NOTIFY is
                 * associated with.
                 * This id parameter MAY be included in NOTIFYs to the first REFER a UA receives
                 * in a given dialog
                 */
        ...
        try {
            ...
            if (uri4transport.isSecure()) {
                ...
                if (transport != null && transport.equalsIgnoreCase(ListeningPoint.UDP)) {
                    throw new SipException(
                            "Cannot create ACK - impossible to use sips uri with transport UDP:" + uri4transport);
                }
            ...
            vias.add(via);
            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                logger.logDebug("Adding via to the ACK we are creating : " + via + " lastResponseTopMostVia "
    ...
    public void setLastResponse(SIPTransaction transaction,
        ...
        try {
            ...
            if (sipResponse.getCSeq().getMethod().equals(Request.INVITE) &&
                    transaction != null && transaction instanceof ClientTransaction
                    && this.getState() != DialogState.TERMINATED) {
     ...
     *                       -- the ACK transaction that was directed at this
     *                       dialog.
    ...
    public boolean handleAck(SIPServerTransaction ackTransaction) {
        ...
        if (isAckSeen() && getRemoteSeqNumber() == ackTransaction.getCSeq()) {
            ...
            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                logger.logDebug("SIPDialog::handleAck: lastResponseCSeqNumber = " + lastInviteOkReceived + " ackTxCSeq "
    ...
    void releaseAckSem() {
        if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
            logger
                    .logDebug("releaseAckSem-enter]]" + this + " sem=" + this.ackSem + " b2bua="
