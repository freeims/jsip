...
import com.github.freeims.freeims.javax.sip.stack.BlockingQueueDispatchAuditor;
import com.github.freeims.freeims.javax.sip.stack.ConnectionOrientedMessageChannel;
import com.github.freeims.freeims.javax.sip.stack.QueuedMessageDispatchBase;
...
public final class PipelinedMsgParser implements Runnable {
     ...
     *                           Message listener which has methods that get called
     *                           back from
     ...
     *
     * @param mlistener
     *                  a SIPMessageListener implementation that can react to
     *                  correct
     *                  and incorrect pars.
     */
    ...
    public class Dispatch implements Runnable, QueuedMessageDispatchBase {
        ...
        public void run() {
            ...
            try {
                ...
                // release the semaphore so that another thread can process another message from
                // the call id queue in the correct order
            ...
            while (isRunning) {
                ...
                while (true) {
                    try {
                        ...
                        if (line1.equals("\n")) {
                            ...
                            if (isPreviousLineCRLF) {
                                ...
                                try {
                                    ...
                                    logger.logError(
                                            "A problem occured while trying to send a single CLRF in response to a double CLRF",
                                            e);
                                }
                                continue;
                            } else {
                        ...
                        if (PostParseExecutorServices.getPostParseExecutor() != null && isRunning) {
                            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
                                logger.logDebug("waiting for messagesOrderingMap " + this + " threadname "
                                        + mythread.getName());
                            synchronized (messagesOrderingMap) {
                                try {
                            ...
                            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
                                logger.logDebug("got notified for messagesOrderingMap " + this + " threadname "
                                        + mythread.getName());
                        }
                ...
                if (maxMessageSize > 0 && contentLength > maxMessageSize)
                    throw new RuntimeException(
                            "Max content size Exceeded! :" + contentLength + " allowed max size is " + maxMessageSize);
                                ...
                                if (orderingStructure == null) {
                                    ...
                                    if (stackLogger.isLoggingEnabled(StackLogger.TRACE_DEBUG)) {
                                        stackLogger.logDebug(
                                                "new CallIDOrderingStructure added for message " + sipMessage);
                                    }
                                }
                            }
    ...
    public void close() {
        ...
        try {
            ...
            if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG))
                logger.logDebug("Couldn't close the rawInputStream " + this + " threadname " + mythread.getName()
                        + " already closed ? " + rawInputStream.isClosed());
            // Ignore.
        }
        if (PostParseExecutorServices.getPostParseExecutor() != null) {
            cleanMessageOrderingMap();
    ...
    private void cleanMessageOrderingMap() {
        ...
        // for (CallIDOrderingStructure callIDOrderingStructure:
        // messagesOrderingMap.values()) {
 ...
 * Fix for TCP calls under load freeze JAIN SIP with
 * TCP_POST_PARSING_THREAD_POOL_SIZE > 0
 *
 * Issue number:
 * Obtained from:
 ...
 * Fix for restarting the stack when the
 * gov.nist.javax.sip.TCP_POST_PARSING_THREAD_POOL_SIZE option is used
 *
 * Issue number:
 * Obtained from:
 * Submitted by: Jean Deruelle
 ...
 * Fixing a deadlock on one post_parser_thread_pool option when there is only 1
 * thread and message ordering on multiple threads + adding non regression test
 * case
 *
 * Issue number:
 * Obtained from:
 ...
 * Fixing Message Order, there could be race conditions on TCP with multiple
 * threads the order should be maintained
 *
 * Issue number: 301
 * Obtained from:
 * Submitted by: Jean Deruelle
 ...
 * Reviewed by: ranga
 *
 * Handle better conflicting multiple settings. Taking into account only the
 * first one without creating unused threadpools for other settings. In trunk.
 *
 ...
 * Big update to improve performance by 50% in some cases, TCK + testsuite
 * (cc-buildloop) green, Mobicents Sip Servlets TCK + testsuite green as well
 *
 * Issue number:
 * Obtained from:
 * Submitted by: Jean Deruelle
