...
*
*/
package com.github.freeims.freeims.javax.sip.address;
...
public class AddressFactoryImpl implements AddressFactoryEx {
     ...
     *                    address. A <code>null</code> value does not set the
     *                    display name.
     ...
     * not be included in the phoneNumber string argument. If scheme is not present,
     * it will be added.
     *
    ...
    public javax.sip.address.URI createURI(String uri) throws ParseException {
        ...
        try {
            ...
            if (!SCHEME_PATTERN.matcher(scheme).matches()) {
                throw new ParseException("the scheme " + scheme + " from the following uri " + uri
                        + " doesn't match ALPHA *(ALPHA / DIGIT / \"+\" / \"-\" / \".\" ) from RFC3261", 0);
            }
        } catch (ParseException ex) {
            throw new ParseException(ex.getMessage(), 0);
        }
        return new com.github.freeims.freeims.javax.sip.address.GenericURI(uri);
    }
