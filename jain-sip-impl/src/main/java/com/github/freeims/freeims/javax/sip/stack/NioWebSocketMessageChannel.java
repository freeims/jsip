...
public class NioWebSocketMessageChannel extends NioTcpMessageChannel {
		...
		if (readingHttp) {
			...
			do {
				...
				if (codec.isCloseOpcodeReceived()) {
					...
					socketChannel.write(byteBuff);// We must skip in the queue, don't use sendNonWebSocketMessage(new
													// byte[]{(byte) 0x88,(byte)0x00}, false);
					return;
				}

				if (decodedMsg == null) {
					return; // the codec can't parse a full websocket frame, we will try again when have
							// more data
				}
	...
	public void processMessage(SIPMessage message) throws Exception {
		if (message instanceof Request) {
			// Commented out for https://java.net/jira/browse/JSIP-504 Contribution by
			// Michael Groshans
			...
			// NioTcpMessageProcessor processor = (NioTcpMessageProcessor)
