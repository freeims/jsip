...
public abstract class AuthenticationHeader extends ParametersHeader {
        ...
        if (nv == null) {
            ...
            if (name.equalsIgnoreCase(ParameterNames.QOP)
                ...
                if (((this instanceof Authorization) || (this instanceof ProxyAuthorization)
                        || (this instanceof WWWAuthenticate))
                /* && name.equalsIgnoreCase(ParameterNames.QOP) */) {
                    // NOP, QOP not quoted in authorization headers
                    // System.out.println("auth header set parameter value NOP");
                    // } else {
                    // System.out.println("auth header set parameter value nv.setQuotedValue();");
                    nv.setQuotedValue();
                }
                if (value == null)
     ...
     * @since v1.1
     *
     *        Note that since 1.2 this is no longer applicable to the
     *        WWW-Authenticate
