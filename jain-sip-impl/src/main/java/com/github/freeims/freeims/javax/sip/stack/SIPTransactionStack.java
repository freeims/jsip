...
import com.github.freeims.freeims.core.net.DefaultNetworkLayer;
...
import com.github.freeims.freeims.javax.sip.*;
 ...
 *
 * @see com.github.freeims.freeims.javax.sip.SipStackImpl
 *
...
public abstract class SIPTransactionStack implements
    ...
    // ThreadPool when parsed SIP messages are processed. Affects the case when many
    // TCP calls use single socket.
     ...
     * a value < 0 means that the RFC 5626 will not be triggered, as a default we
     * don't enable it not to change existing apps behavior.
     */
        ...
        if (getMessageProcessorFactory() instanceof NioMessageProcessorFactory) {
            ...
            for (MessageProcessor processor : processors) {
                if ("TCP".equals(processor.getTransport())) {
                    NioTcpMessageChannel msgChannel = (NioTcpMessageChannel) processor.createMessageChannel(dst,
                            dstPort);
    ...
    public boolean removeTransactionPendingAck(SIPServerTransaction serverTransaction) {
        // String branchId =
        // ((SIPRequest)serverTransaction.getRequest()).getTopmostVia().getBranch();
        String branchId = serverTransaction.getBranchId();
        if (branchId != null
                && this.terminatedServerTransactionsPendingAck.containsKey(branchId)) {
                ...
                if (logger.isLoggingEnabled(LogWriter.TRACE_ERROR)) {
                    logger.logError(
                            "An issue happening the valve on request " +
                                    requestReceived +
                                    " thus the message will not be processed further",
                            e);
                }
                return null;
            }
            ...
            if (sipTransaction instanceof SIPServerTransaction) {
                ...
                if (removed != null) {
                    ...
                    if (forkId != null && clientTx.isInviteTransaction()
                        ...
                        if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                            logger.logDebug(
                                    "Scheduling to remove forked client transaction : forkId = " + forkId + " in "
                                            + this.maxForkTime + " seconds");
                        }
     ...
     * process them in single thread.
     * This property allows immediately after parsing to split the load into many
     * threads which increases
     * the performance significantly. If set to 0 then we just use the old model
     * with single thread.
     *
     * @return
     */
     ...
     *                       -- the header that references the dialog being
     *                       replaced.
     */
    ...
    public SipTimer getTimer() throws IllegalStateException {
        // if(timer == null)
        // throw new IllegalStateException("Stack has been stopped, no further tasks can
        // be scheduled.");
        return timer;
    }
    ...
    public void setReliableConnectionKeepAliveTimeout(int reliableConnectionKeepAliveTimeout) {
        ...
        // throw new IllegalArgumentException("The Stack
        // reliableConnectionKeepAliveTimeout can not be negative.
        // reliableConnectionKeepAliveTimeoutCandidate = " +
    ...
    /**
     * Find suitable MessageProcessor and calls it's
     * {@link MessageProcessor#closeReliableConnection(String, int)}
