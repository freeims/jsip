...
import com.github.freeims.freeims.javax.sip.header.ReferTo;
...
import com.github.freeims.freeims.javax.sip.stack.MessageChannel;
import com.github.freeims.freeims.javax.sip.stack.SIPClientTransaction;
import com.github.freeims.freeims.javax.sip.stack.SIPDialog;
import com.github.freeims.freeims.javax.sip.stack.SIPServerTransaction;
import com.github.freeims.freeims.javax.sip.stack.SIPTransaction;
import com.github.freeims.freeims.javax.sip.stack.SIPTransactionStack;
import com.github.freeims.freeims.javax.sip.stack.ServerRequestInterface;
import com.github.freeims.freeims.javax.sip.stack.ServerResponseInterface;
...
class DialogFilter implements ServerRequestInterface, ServerResponseInterface {
     ...
     *                               is thrown when there is an error processing the
     *                               request.
     */
    public void processRequest(SIPRequest sipRequest,
        ...
        if (sipRequestMethod.equals(Request.REFER) && dialog != null
            ...
            SIPTransaction lastTransaction = dialog == null ? null
                    : dialog
             ...
             */
            lastTransaction = (dialog == null ? null
                    : dialog
                            .getLastTransaction());
            ...
            if (dialog != null
                ...
                if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
                    logger.logDebug(
                            "DialogFilter::processRequest:lastTransaction.getState(): " + lastTransaction.getState() +
     ...
     *                               -- message channel on which the response is
     *                               received.
     */
            ...
            if (sipStack.getMaxForkTime() != 0
                ...
                if (dialog != null && forked != null) {
                    ...
                    if (forked.getDefaultDialog() != null && !dialog.equals(forked.getDefaultDialog())) {
                        if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
                            logger.logDebug(
                                    "forkedId= " + response.getForkId() + " forked dialog " + dialog + " original tx "
                        ...
                        if (transaction == null && dialog.getState() == DialogState.EARLY
                            ...
                            // where some responses are still able to be handled by it so we update the
                            // dialog to CONFIRMED in setting the last response so the ACK can be created by
                            // the application
    ...
    public void processResponse(SIPResponse sipResponse,
        ...
        if (SIPTransactionStack.isDialogCreated(method)
            ...
            // Issue 317 : for forked response even if automatic dialog support is not
            // enabled
            // a dialog should be created in the case where the original Tx already have a
            // default dialog
            // and the current dialog is null. This is also avoiding creating dialog
            // automatically if the flag is not set
            if (sipProvider.isAutomaticDialogSupportEnabled()) {
                createDialog = true;
            } else {
        ...
        if (sipStack.getMaxForkTime() != 0
            ...
            if (sipDialog != null && originalTx != null) {
                ...
                if (originalTx.getDefaultDialog() != null && !sipDialog.equals(originalTx.getDefaultDialog())) {
                    if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
                        logger.logDebug(
                                "forkedId= " + sipResponse.getForkId() + " forked dialog " + sipDialog + " original tx "
                                        + originalTx + " original dialog " + originalTx.getDefaultDialog());
                    }
