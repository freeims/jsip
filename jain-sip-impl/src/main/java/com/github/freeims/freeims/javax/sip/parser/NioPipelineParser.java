...
public class NioPipelineParser {
	...
	boolean partialLineRead = false; // if we didn't receive enough bytes for a full line we expect the line to end
										// in the next batch of bytes
	String partialLine = "";
	String callId;
			...
			try {
				...
				if (!acquired) {
					if (logger.isLoggingEnabled(StackLogger.TRACE_WARN)) {
						logger.logWarning("Semaphore acquisition for callId " + callId
								+ " wasn't successful so don't process message, returning");
					}
					// https://java.net/jira/browse/JSIP-499 don't process the message if the
					// semaphore wasn't acquired
					return;
				} else {
				...
				logger.logError("Semaphore acquisition for callId " + callId
						+ " interrupted, couldn't process message, returning", e);
				// https://java.net/jira/browse/JSIP-499 don't process the message if the
				// semaphore wasn't acquired
				return;
			}
			...
			try {
				synchronized (smp) {
					...
					if (parsedSIPMessage == null) {
						...
						if (logger.isLoggingEnabled(StackLogger.TRACE_DEBUG)) {
							logger.logDebug(
									"parsed message is null, probably because of end of stream, empty packets or socket closed "
				...
				if (!messagePolled) {
					...
					// wasn't polled in case of some exception by example while parsing or before
	 ...
	 * message structure.
	 * For TCP the key things to identify are message lines for the headers, parse
	 * the Content-Length header
	 * and then read the message body (aka message content). For TCP the
	 * Content-Length must be 100% accurate.
	 */
	...
	private boolean readMessageSipHeaderLines(InputStream inputStream, boolean isPreviousLineCRLF) throws IOException {
		...
		if (partialLineRead) {
			...
			if (!line.equals(CRLF)) { // CRLF indicates END of message headers by RFC
				...
				// contribution from Alexander Saveliev compare to lower case as RFC 3261 states
				...
				if (lineIgnoreCase.startsWith(ContentLengthHeader.NAME.toLowerCase())) { // naive Content-Length header
																							...
																							// much bytes of message
																							// body must be read after
																							// the SIP headers
				...
				if (isPreviousLineCRLF) {
					...
					try {
						...
						logger.logError(
								"A problem occured while trying to send a single CLRF in response to a double CLRF", e);
					}
				} else {
					crlfReceived = true;
	...
	private void readMessageBody(InputStream inputStream) throws IOException {
		...
		if (contentReadSoFar == contentLength) { // We have read the full message headers + body
			...
			if (PostParseExecutorServices.getPostParseExecutor() != null) {
				...
				PostParseExecutorServices.getPostParseExecutor().execute(new Dispatch(callIDOrderingStructure, callId)); // run
																															// in
																															// executor
																															// thread
			} else {
