 ...
 * handle each new
 * connection. This is the active object that creates new TLS MessageChannels
 * (one for each new
 * accept socket).
 * 
...
public class TLSMessageProcessor extends ConnectionOrientedMessageProcessor implements Runnable {
    ...
    public void start() throws IOException {
        ...
        if (sipStack.getClientAuth() == ClientAuthType.Want || sipStack.getClientAuth() == ClientAuthType.Default) {
            // we set it to true in Default case as well to keep backward compatibility and
            // default behavior
    ...
    public void run() {
        ...
        while (this.isRunning) {
            ...
            if (isRunning) {
                try {
                    // lyolik: even if SocketException is thrown (could be a result of bad
                    // handshake,
                    ...
                    // https://code.google.com/p/jain-sip/issues/detail?id=14 add it only if the
