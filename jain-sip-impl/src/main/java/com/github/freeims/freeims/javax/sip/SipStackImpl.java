 ...
 *
 */
package com.github.freeims.freeims.javax.sip;
...
import com.github.freeims.freeims.core.net.AddressResolver;
import com.github.freeims.freeims.core.net.DefaultSecurityManagerProvider;
import com.github.freeims.freeims.core.net.NetworkLayer;
import com.github.freeims.freeims.core.net.SecurityManagerProvider;
import com.github.freeims.freeims.core.net.SslNetworkLayer;
import com.github.freeims.freeims.javax.sip.clientauthutils.AccountManager;
import com.github.freeims.freeims.javax.sip.clientauthutils.AuthenticationHelper;
import com.github.freeims.freeims.javax.sip.clientauthutils.AuthenticationHelperImpl;
import com.github.freeims.freeims.javax.sip.clientauthutils.SecureAccountManager;
import com.github.freeims.freeims.javax.sip.parser.MessageParserFactory;
import com.github.freeims.freeims.javax.sip.parser.PostParseExecutorServices;
import com.github.freeims.freeims.javax.sip.parser.StringMsgParser;
import com.github.freeims.freeims.javax.sip.parser.StringMsgParserFactory;
import com.github.freeims.freeims.javax.sip.stack.ByteBufferFactory;
import com.github.freeims.freeims.javax.sip.stack.ClientAuthType;
import com.github.freeims.freeims.javax.sip.stack.ConnectionOrientedMessageProcessor;
import com.github.freeims.freeims.javax.sip.stack.DefaultMessageLogFactory;
import com.github.freeims.freeims.javax.sip.stack.DefaultRouter;
import com.github.freeims.freeims.javax.sip.stack.MessageProcessor;
import com.github.freeims.freeims.javax.sip.stack.MessageProcessorFactory;
import com.github.freeims.freeims.javax.sip.stack.OIOMessageProcessorFactory;
import com.github.freeims.freeims.javax.sip.stack.SIPEventInterceptor;
import com.github.freeims.freeims.javax.sip.stack.SIPMessageValve;
import com.github.freeims.freeims.javax.sip.stack.SIPTransactionStack;
import com.github.freeims.freeims.javax.sip.stack.SocketTimeoutAuditor;
import com.github.freeims.freeims.javax.sip.stack.timers.DefaultSipTimer;
import com.github.freeims.freeims.javax.sip.stack.timers.SipTimer;
 ...
 * header field,
 * the UAS core MUST check the request against ongoing transactions. If the From
 * tag, Call-ID, and CSeq
 ...
 * Dialog in the
 * NULL state is terminated. ( Dialogs in the NULL state are not associated with
 * an actual SIP Dialog.
 * They are a programming convenience. A Dialog is in the NULL state before the
 * first response for the
 * Dialog forming Transaction). If set to true, the SipListener will get a
 ...
 * result in a unusable thread stuck waiting for the lock to be released. A good
 * value
 * for this property is the lifespan of the transaction or the expected blocking
 * delay in
 * the listener.
 * 
 ...
 * This is to avoid any leaks in whatever state the transaction can be in even
 * if the application misbehaved
 ...
 * specific property as a non INVITE property is short live as compared to
 * INVITE
 ...
 * load
 * of new calls that hang up or stay idle. Note that disconnecting the socket
 * does't
 * end the SIP call. A new socket will be established when needed for any
 * existing calls
 * by the SIP RFC spec.
 * </li>
 * 
 ...
 * direct buffers.
 * As Direct buffers reside outside of the heap memory, they can lead to
 * unforeseen out of memory exceptions
 * as seen in http://java.net/jira/browse/JSIP-430. This flag allows to use non
 * direct buffers for better memory
 ...
 * messages. It
 * cannot recover once a malformed message is processed, because the stream
 * iterator
 ...
 * When a forked response is received in this time interval from when the
 * original
 * INVITE client transaction was sent, the stack will place the original INVITE
 * client transction in the ResponseEventExt and deliver that to the
 * application.
 ...
 * on how and when it accesses transaction and dialog data since it cleans up
 ...
 * with CRLF CRLF if greater they will be rejected. The default is -1 (i.e. do
 ...
 * message
 * before any long-lived SIP Stack resources are allocated (no transactions, no
 * dialogs). From within the processRequest callback
 * implementation you can drop messages, send a response statelessly or
 * otherwise transform/pre-process the message before it reaches
 * the next steps of the pipeline. Similarly from processResponse() you can
 * manipulate a response or drop it silently, but dropping
 * responses is not recommended, because the transaction already exists when the
 ...
 * occur only on certain events, so it will not overwhelm the CPU. The overall
 * performance penalty by using this class in
 ...
 * It's advisable not to use SSL protocols because of
 ...
 * accepting a connection. Set to Want if you want the SSL stack to request a
 * client Certificate, but not fail if one isn't presented.
 * A Disabled value will not require a certificate chain for the Server
 * Connection. A DisabledAll will not require a certificate chain for both
...
public class SipStackImpl extends SIPTransactionStack implements
	...
	private String[] enabledProtocols = {
			// https://java.net/jira/browse/JSIP-482 defaulting only TLS protocols and not
			// SSL anymore
			"TLSv1.2",
			"TLSv1.1",
			"TLSv1"
	 ...
	 *                                -- stack configuration properties including
	 *                                NIST-specific
	 *                                extensions.
	...
	public SipStackImpl(Properties configurationProperties)
		...
		if (tcpTreadPoolSize != null) {
			try {
				...
				if (logger.isLoggingEnabled())
					this.logger.logError(
							"TCP post-parse thread pool size - bad value " + tcpTreadPoolSize + " : "
									+ ex.getMessage());
			}
		}
	...
	public void addLogAppender(org.apache.log4j.Appender appender) {
		if (this.logger instanceof com.github.freeims.freeims.core.LogWriter) {
			((com.github.freeims.freeims.core.LogWriter) this.logger).addAppender(appender);
		}
	}
	...
	public org.apache.log4j.Logger getLogger() {
		if (this.logger instanceof com.github.freeims.freeims.core.LogWriter) {
			return ((com.github.freeims.freeims.core.LogWriter) this.logger).getLogger();
		}
		return null;
	}
	 ...
	 * 
	 * @param String
	 *               [] The new set of protocols to use for outgoing TLS
	 *               connections.
	 * @return
	 * 
	 */
