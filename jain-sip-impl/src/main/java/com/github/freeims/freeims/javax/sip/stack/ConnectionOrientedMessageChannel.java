...
import com.github.freeims.freeims.javax.sip.IOExceptionEventExt;
...
import com.github.freeims.freeims.javax.sip.IOExceptionEventExt.Reason;
...
import com.github.freeims.freeims.javax.sip.parser.Pipeline;
import com.github.freeims.freeims.javax.sip.parser.PipelinedMsgParser;
import com.github.freeims.freeims.javax.sip.parser.SIPMessageListener;
...
public abstract class ConnectionOrientedMessageChannel extends MessageChannel implements
     ...
     *
     * @param sipMessage
     *                   Message to process (this calls the application for
     *                   processing
     *                   the message).
     *
    ...
    public void cancelPingKeepAliveTimeoutTaskIfStarted() {
        if (pingKeepAliveTimeoutTask != null && pingKeepAliveTimeoutTask.getSipTimerTask() != null) {
            ...
            try {
                if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                    logger.logDebug("~~~ cancelPingKeepAliveTimeoutTaskIfStarted for MessageChannel(key=" + key
    ...
    public void rescheduleKeepAliveTimeout(long newKeepAliveTimeout) {
        ...
        if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
            methodLog.append(
                    "~~~ rescheduleKeepAliveTimeout for MessageChannel(key=" + key + "), clientAddress=" + peerAddress
        ...
        try {
            if (pingKeepAliveTimeoutTask == null) {
                ...
                if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
                    logger.logDebug("~~~ cancelPingKeepAliveTimeout for MessageChannel(key=" + key + "), clientAddress="
                            + peerAddress
                            + ", clientPort=" + peerPort + ", timeout=" + keepAliveTimeout + ")");
                }
                sipStack.getTimer().cancel(pingKeepAliveTimeoutTask);
