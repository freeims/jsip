...
/**
 * This is a helper state machine that negotiates the SSL connection
 * automatically
 ...
 * 
 * Other than that the state machine is able to handle partial chunks of SIP
 * messages
 * and only supply them when they are ready to the original TCP channel once
 * they are
 * decrypted.
 * 
...
public class SSLStateMachine {
	...
	public void wrap(ByteBuffer src, ByteBuffer dst,
		...
		synchronized (wrapLock) {
			...
			// Null src means we just have no ne data but still want to push any previously
			// queued data
			if (src != null) {
			...
			loop: while (true) {
				...
				try {
					...
					if (!currentBuffer.message.hasRemaining()) {
						...
						if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
							logger.logDebug(
									"REMOVED item from encryption queue because it has no more data, all is done, buffers size now is "
				...
				if (result.bytesProduced() > 0) {
					// produced > 0 means encryption was successful and we have something to send
					// over the wire
					dst.flip();
					byte[] bytes = new byte[dst.remaining()];
					dst.get(bytes);
					if (currentBuffer.getCallBack() != null) {
						// Send using message channel (it discriminates between client/server and
						// new/old connecitons)
						currentBuffer.getCallBack().doSend(bytes);
					} else {
						// Send using the existing connection without attempting to guess client or
						// server etc
							...
							if (channel instanceof NioTlsMessageChannel) {
								...
								if (sslEngine.getSession() != null) {
									if (!ClientAuthType.Disabled.equals(channel.getSIPStack().getClientAuth())
											&& !ClientAuthType.DisabledAll
													.equals(channel.getSIPStack().getClientAuth())) {
										// https://java.net/jira/browse/JSIP-483 Don't try to get the PeerCertificates
										// if the client auth is Disabled or DisabledAll as they won't be available
										try {
											...
											// no op if -Dgov.nist.javax.sip.TLS_CLIENT_AUTH_TYPE=Disabled is used, no
											// peer certificates will be available
											if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
												logger.logDebug(
														"sslEngine.getSession().getPeerCertificates() are not available, which is normal if running with gov.nist.javax.sip.TLS_CLIENT_AUTH_TYPE=Disabled");
											}
										}
									}
									((NioTlsMessageChannel) channel).getHandshakeCompletedListener()
							...
							if (sslEngine.getSession() != null) {
								if (!ClientAuthType.Disabled.equals(channel.getSIPStack().getClientAuth())
									...
									try {
										...
										// no op if -Dgov.nist.javax.sip.TLS_CLIENT_AUTH_TYPE=Disabled is used, no peer
	...
	private void startBuffer(ByteBuffer src) {
		if (tlsRecordBuffer == null) {
			...
			// Begin buffering, if there is already a buffer the normalization will take of
			// adding the bytes
			tlsRecordBuffer = ByteBufferFactory.getInstance().allocateDirect(33270); // max record size in other
																						// implementations
		...
		synchronized (unwrapLock) {
			...
			loop: while (true) {
				...
				try {
					...
					// Make sure to throw the exception so the result variable is not null below
					// which makes the stack hang
					if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				...
				if (result.getStatus().equals(Status.BUFFER_UNDERFLOW)) {
					if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
						logger.logDebug(
								"Buffer underflow, wait for the next inbound chunk of data to feed the SSL engine");
					}
					startBuffer(src);
					break;
				} else {
				...
				if (result.getStatus().equals(Status.BUFFER_OVERFLOW)) {
					...
					ByteBuffer newBuf = channel
							.prepareAppDataBuffer(sslEngine.getSession().getPacketBufferSize() + dst.position());
					dst.flip();
					newBuf.put(dst);
					dst = newBuf;
					if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
				...
				switch (result.getHandshakeStatus()) {
					...
					case FINISHED:
						if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
							logger.logDebug(
									"Handshaking just finnished, but has remaining. Will try to wrap the queues app items.");
						}
						wrapRemaining();
						if (src.hasRemaining()) {
							break;
					...
					case NOT_HANDSHAKING:
						...
						if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
							logger.logDebug(
									"Not handshaking, but has remaining: " + src.hasRemaining() + " buffer " + src);
						}
						if (src.hasRemaining()) {
							break;
						} else {
	...
	private void runDelegatedTasks(SSLEngineResult result) throws IOException {
		 ...
		 * Call them every time they have NEED_TASK otherwise the sslEngine won't make
		 * progress
		 */
	...
	/**
	 * Each time we send a SIP message it will be associated with the originating
	 * channel.send() method
	 * which keep additional information about the transport in its context. That
