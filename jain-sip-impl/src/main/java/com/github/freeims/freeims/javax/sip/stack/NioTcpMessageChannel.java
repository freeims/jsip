...
import com.github.freeims.freeims.javax.sip.parser.NioPipelineParser;
...
public class NioTcpMessageChannel extends ConnectionOrientedMessageChannel {
	...
	public void readChannel() {
		...
		try {
			...
			// This prevents us from getting stuck in a selector thread spinloop when socket
			// is constantly ready for reading but there are no bytes.
			if (nbytes == 0)
			...
			try {
				if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
					logger.logDebug("IOException  closing sock " + ex + "myAddress:myport " + myAddress + ":" + myPort
							+ ", remoteAddress:remotePort " + peerAddress + ":" + peerPort);
				}
	...
	public void sendTCPMessage(byte message[], InetAddress receiverAddress,
		...
		if (sock != socketChannel && sock != null) {
			if (socketChannel != null) {
				...
				close(false, false); // we can call socketChannel.close() directly but we better use the inherited
										// method
